...
void CefBrowserHostWrapper::SendKeyEvent(int message, int wParam, int lParam)
{
    ...
    if (((keyEvent.type == KEYEVENT_CHAR) && IsKeyDown(VK_CONTROL) && IsKeyDown(VK_SHIFT) && IsKeyDown(VK_RMENU)) || // LEFT SHIFT + ALT GR
        ((keyEvent.type == KEYEVENT_CHAR) && IsKeyDown(VK_CONTROL) && IsKeyDown(VK_LMENU))) // LEFT CONTROL + LEFT ALT
    {
        keyEvent.modifiers &= ~(EVENTFLAG_CONTROL_DOWN | EVENTFLAG_ALT_DOWN);
        keyEvent.modifiers |= EVENTFLAG_ALTGR_DOWN;
    }
    else if ((keyEvent.type == KEYEVENT_CHAR) && IsKeyDown(VK_RMENU)) {
